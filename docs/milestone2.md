...
User marks off each category in the ratings section and clicks 'Submit rating, which will then create a new rating submission. In addition, user can write a review and when 'Submit review' which will create a new review. 
* DELETE
...
User can delete their account when they click "Delete account" at the bottom of the screen.
* READ
...
Data is read from the database which populates each course card as shown above. Users are able to view and filter through the cards.
* UPDATE 
...
Users click on the thumbs up or thumbs down icons in order to rate each review. Once clicked, the number next to the icons will update and increment. 
